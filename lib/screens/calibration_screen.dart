  ...
  class CalibrationPair {
    ...
    CalibrationPair(this.src, this.z, this.dst);
  }
  ...
  class CalibrationScreenState extends State<CalibrationScreen> {
    ...
    void _enterValidationMode() {
      try {
        ...
        // 1. 새 솔버 호출
        final hMatrix = HomographySolver.solve(src, zs, dst);
    ...
    void _updatePreviewPoints() {
      ...
      for (int i = 0; i < collectedPairs.length; i++) {
        ...
        // VisualPull 없이 순수 transform 결과만 확인
        Offset mathRes = CoordinateTransformer.transform(p.src.dx, p.src.dy, p.z);

        newPoints.add(mathRes);
        newErrors.add((mathRes - targetPoints[i]).distance);
      }
    ...
    Widget _buildActionTrigger({
      ...
      // 카운트다운 중일 때만 액티브 색상을 쓰고, 아니면 희미한 흰색(대기) 처리
